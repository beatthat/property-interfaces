...
namespace BeatThat
{
	...
	namespace PropertyExt 
	{
		...
		public static class PropertyExtensions
		{
			...
			private static bool DEBUG_GET_VALUE_TYPE = 
				#if DEBUG_PropertyExt_GetValueType
				true;
				#else
				false;
				#endif
			...
			/// <summary>
			/// Gets the value type T from interface IHasValue<T> from passed in type.
			/// </summary>
			public static bool GetValueType(this Type t, out Type valueType)
			{
				valueType = null;

                // if param t is itself IHasValue<T> then just use t, otherwise look for IHasValue<T> in t's interfaces
				var iHasValueInterface = (t.IsInterface && t.IsGenericType && t.GetGenericTypeDefinition() == typeof(IHasValue<>))?
					t : Array.Find(t.GetInterfaces(), intf => intf.IsGenericType && typeof(IHasValue<>).IsAssignableFrom(intf.GetGenericTypeDefinition()));

				if (iHasValueInterface == null) {
					#if UNITY_EDITOR || DEBUG_UNSTRIP
					if(DEBUG_GET_VALUE_TYPE) {
						Debug.Log("[" + Time.frameCount + "] GetValueType for type " + t.Name + ": no IHasValue<> interface");
					}
					#endif
					return false;
				}
				...
				if (iHasValueInterface.ContainsGenericParameters) {
					#if UNITY_EDITOR || DEBUG_UNSTRIP
					if(DEBUG_GET_VALUE_TYPE) {
						Debug.Log("[" + Time.frameCount + "] GetValueType for type " + t.Name + ": IHasValue<> interface has no params");
					}
					#endif
					return false;
				}
				...
				#if UNITY_EDITOR || DEBUG_UNSTRIP
				if(DEBUG_GET_VALUE_TYPE) {
					Debug.Log("[" + Time.frameCount + "] GetValueType for type " + t.Name + ": value type=" + valueType.Name);
				}
				#endif
				return true;
			}
